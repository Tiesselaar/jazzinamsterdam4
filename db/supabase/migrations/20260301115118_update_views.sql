-- 0_drop_views.sql
drop view view_gigs;
drop view view_gigs_with_display_date;
drop view view_gigs_with_time;
drop view view_timed_calendars;
  ...
  *,
  case
    when calendar_type = 'concert' then (
      now () at time zone time_zone - interval '13 hours'
    )::date
    else (now () at time zone time_zone)::date
  end as current_display_day
from
  calendars
